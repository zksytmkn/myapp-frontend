  ...
  methods: {
    ...
    addProductToCart(id, quantity) {
      if(!this.$store.state.carts.length || !this.$store.state.carts.some(cart => cart.product_id === id)) {
        const asyncFunc = async() => {
          const productQuantity = Number(this.$store.state.product.favorite.find(product => product.id === id).stock) - Number(quantity)
          ...
          .then(response => {
            ...
            this.$store.dispatch('getProductFavorite', response[1])
          })
        }
        asyncFunc().finally(response => console.log(response))
      }
